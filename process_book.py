...
def print_page_num(b, encoding='utf-8'):
    try:
...
# Main function
def file_to_wikitext(inputFilename, language='en-GB', debug=False):
    ...
    with open(os.path.join('books', outputFilename), 'w', encoding=ENCODING) as out:
        out.write(text.strip())
    print('\nSneak peek:\n\n' + text.strip()[:200] + '...')

if __name__ == '__main__':
    language = sys.argv[2] if len(sys.argv) > 2 else 'en-GB'
    file_to_wikitext(sys.argv[1], language=language, debug=True)
